    ...
    CBigInteger operator+(CBigInteger &c){	
    	...
    	if(is_p^c.is_p==0){
    		...
    		for(int i=0;i<minn;i++){
    			if(i>=n)ans.num[i]+=c.num[i]-'0';
    			else if(i>=c.n)ans.num[i]+=num[i]-'0';
    			else ans.num[i]=ans.num[i]+num[i]-'0'+c.num[i]-'0';
			...
			if(n>c.n||(n==c.n&&num[n-1]>c.num[n-1])){
				for(int i=0;i<min(n,c.n);i++){
					ans.num[i]=ans.num[i]+num[i]-c.num[i];
					//cout<<ans.num[i]<<endl;
					if(ans.num[i]<'0'){
						ans.num[i]+=10;
						ans.num[i+1]--;
